...
int event_loop_handle_pending_add(struct event_loop *eventLoop, int fd, struct channel *channel) {
    printf("add channel fd == %d, %s", fd, eventLoop->thread_name);
...
int event_loop_handle_pending_update(struct event_loop *eventLoop, int fd, struct channel *channel) {
    printf("update channel fd == %d, %s", fd, eventLoop->thread_name);
    struct channel_map *map = eventLoop->channelMap;
...
int handleWakeup(void *data) {
    ...
    printf("wakeup, %s", eventLoop->thread_name);
}
...
struct event_loop *event_loop_init_with_name(char *thread_name) {
    ...
    printf("set poll as dispatcher, %s", eventLoop->thread_name);
...
int event_loop_run(struct event_loop *eventLoop) {
    ...
    printf("event loop run, %s", eventLoop->thread_name);
    ...
    printf("event loop end, %s", eventLoop->thread_name);
    return 0;
}
